...
class SpaCySettings(BaseSettings):
    ...
    english_model: str = Field(default="en_core_web_trf", description="English transformer model")
    multilingual_model: str = Field(default="xx_sent_ud_sm", description="Multilingual model")
...
class StanzaSettings(BaseSettings):
    ...
    languages: List[str] = Field(default=["en", "hi", "gu", "ur"], description="Languages to load")
    processors: str = Field(default="tokenize,pos,lemma,depparse", description="Processors to load")
...
class IndicNLPSettings(BaseSettings):
    ...
    languages: List[str] = Field(default=["hi", "gu", "mr", "bn", "ta", "te", "ur"], description="Indic languages")
...
class AnalysisSettings(BaseSettings):
    ...
    default_strictness: int = Field(default=7, ge=1, le=10, description="Default strictness level")
    ...
    enable_batch_processing: bool = Field(default=True, description="Enable batch analysis")
...
class RatingWeights(BaseSettings):
    ...
    language_diction: float = Field(default=0.15, description="Language & diction weight")
    ...
    cultural_fidelity: float = Field(default=0.15, description="Cultural fidelity weight")
    originality: float = Field(default=0.15, description="Originality weight")
    computational_greatness: float = Field(default=0.10, description="Computational greatness weight")
...
class ProsodySettings(BaseSettings):
    ...
    hindi_chhands: List[str] = Field(
        default=["doha", "chaupai", "soratha", "kundaliya", "rola", "harigitika", "barvai", "savaiya", "kavitt"],
...
class RasaSettings(BaseSettings):
    ...
    model_path: Optional[str] = Field(default=None, description="Path to Rasa classification model")
...
class TouchstoneSettings(BaseSettings):
    ...
    english_touchstones: List[Dict[str, str]] = Field(
        default=[
            ...
            {"name": "Shakespeare", "passage": "Shall I compare thee to a summer's day"},
            {"name": "Homer", "passage": "Sing goddess the anger of Peleus son Achilles"},
...
class ConstraintSettings(BaseSettings):
    ...
    dictionary_path: Optional[str] = Field(default=None, description="Path to dictionary for N+7")
...
class VisualizationSettings(BaseSettings):
    ...
    enable_heatmaps: bool = Field(default=True, description="Enable heatmap visualization")
    enable_network_graphs: bool = Field(default=True, description="Enable network graphs")
    chart_library: str = Field(default="plotly", description="Chart library (plotly/matplotlib)")
    output_format: str = Field(default="html", description="Output format for visualizations")
...
class AppSettings(BaseSettings):
    ...
    app_name: str = Field(default="Ultimate Literary & Linguistic Master System", description="Application name")
    ...
    database_url: str = Field(default="sqlite:///./literary_analysis.db", description="Database URL")
